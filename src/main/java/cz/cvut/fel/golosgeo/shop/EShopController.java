...
import cz.cvut.fel.golosgeo.archive.PurchasesArchive;
import cz.cvut.fel.golosgeo.storage.NoItemInStorage;
import cz.cvut.fel.golosgeo.storage.Storage;
...
public class EShopController {
    ...
    public static void purchaseShoppingCart(ShoppingCart cart, String customerName, String customerAddress) throws NoItemInStorage, NoItemInStorage {
    ...
    public static void insertItems(Item[] storageItems, int[] itemCount){
        for (int i = 0; i < storageItems.length; i++) {
            storage.insertItems(storageItems[i], itemCount[i]);
        }
    }

    public static void makeStorageEmptyOnlyForTests(){
        storage = null;
    }
